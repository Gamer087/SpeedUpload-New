...
def processUploadFiles(filename,filesize,files,update,bot,message,thread=None,jdb=None):
    try:
        ...
        if cloudtype == 'moodle':
            ...
            if user_info['tokenize']!=0:
               tokenize = True
            bot.editMessageText(message,'🚀Subiendo PorFavor Espere')
    ...
    if client:
        ...
        try:
            ...
            if getUser['uploadtype'] == 'calendar' or getUser['uploadtype'] == 'draft':
                ...
                bot.sendMessage(message.chat.id,'📅Enlaces Movidos a Calendario🔗')
                bot.sendFile(update.message.chat.id,fname)
            else:
                return
        except:
            bot.sendMessage(message.chat.id,'⚠️Error al Mover a Calendario⚠️')
    else:
        bot.editMessageText(message,'⚠️Error en la Nube⚠️')
...
def onmessage(update,bot:ObigramClient):
    try:
        ...
        if username == tl_admin_user or user_info:  # validate user
            ...
            mensaje = "📋Usted no tiene acceso a este bot.\n🧬Contacta a mi desarrollador: @TuguerX\n"
        ...
        if '/add' in msgText:
            ...
            if isadmin:
                try:
                    ...
                    jdb.save()
                    msg = '👤@'+user+' ha sido añadido al bot como [USUARIO]'
                    bot.sendMessage(update.message.chat.id,msg)
                except:
                    bot.sendMessage(update.message.chat.id,f'⚠️Error en el comando /add usuario')
            else:
                bot.sendMessage(update.message.chat.id,'⚠️Lo sentimos usted no tiene rango de administrador')
            return
        if '/admin' in msgText:
            isadmin = jdb.is_admin(username)
            if isadmin:
                try:
                    ...
                    jdb.save()
                    msg = '👑@'+user+' ha sido añadido al bot como [ADMIN]'
                    bot.sendMessage(update.message.chat.id,msg)
                except:
        ...
        if '/prueba' in msgText:
            ...
            if isadmin:
                try:
                    ...
                    jdb.save()
                    msg = '👤@'+user+' ha sido añadido al bot en [MODO PRUEBA]'
                    bot.sendMessage(update.message.chat.id,msg)
                except:
                    bot.sendMessage(update.message.chat.id,f'⚠️Error en el comando /prueba usuario⚠️')
            else:
                bot.sendMessage(update.message.chat.id,'⚠️Lo sentimos usted no tiene rango de administrador')
            return 
        if '/ban' in msgText:
            ...
            if isadmin:
                try:
                    ...
                    jdb.remove(user)
                    jdb.save()
                    msg = '❌El usuario '+user+' ha sido expulsado🚫'
                    bot.sendMessage(update.message.chat.id,msg)
                except:
                ...
                bot.sendMessage(update.message.chat.id,'⚠️Lo sentimos usted no tiene rango de administrador')
            return
        if '/tuguer087' in msgText:
            isadmin = jdb.is_admin(username)
            if isadmin:
                sms1 = bot.sendMessage(update.message.chat.id,'⏫Enviando database...')
        ...
        if '/tuguer087' in msgText:
            ...
            if isadmin:
                ...
                bot.sendMessage(update.message.chat.id,'⚠️Lo sentimos usted no tiene rango de administrador')
            return
        if '/userinfo' in msgText:
            isadmin = jdb.is_admin(username)
            if isadmin:
                message = bot.sendMessage(update.message.chat.id,'📄')
                message = bot.sendMessage(update.message.chat.id,'👑Eres administrador del bot, tienes control total sobre el mismo✅')
            else:
                message = bot.sendMessage(update.message.chat.id,'📄')
                message = bot.sendMessage(update.message.chat.id,'😕Eres usuario, tienes control parcialmente sobre el bot❎')
            return
        # end
        ...
        if '/delChannel' in msgText:
            ...
            if getUser:
                ...
                bot.sendMessage(update.message.chat.id,statInfo)
            return
        if '/acerca' in msgText:
            message = bot.sendMessage(update.message.chat.id,'📄')
            información = open('información.txt','r')
            bot.sendMessage(update.message.chat.id,información.read())
            información.close()
            return
        if '/comandos' in msgText:
            message = bot.sendMessage(update.message.chat.id,'📄/setcommands en @BotFather')
        ...
        if '/zips' in msgText:
            ...
            if getUser:
                try:
                   ...
                   jdb.save_data_user(username,getUser)
                   jdb.save()
                   msg = '🗜Se ha establecido un tamaño de '+ sizeof_fmt(size*1024*1024)+' por parte📚'
                   bot.sendMessage(update.message.chat.id,msg)
                except:
        ...
        if '/up' in msgText:
            try:
                ...
                bot.sendMessage(update.message.chat.id,'⚠️Error en el comando /up tipo de subida (evidence,draft,blog,calendar,perfil)⚠️')
            return
        ...
        if '/off_proxy' in msgText:
            try:
                ...
                if getUser:
                    ...
                    jdb.save_data_user(username,getUser)
                    jdb.save()
                    msg = '🔌Proxy desequipado exitosamente.\n'
                    bot.sendMessage(update.message.chat.id,msg)
            except:
                if user_info:
                    user_info['proxy'] = ''
        ...
        if '/start' in msgText:
            ...
            start_msg+= '✅Activar comandos /comandos \n'
            start_msg+= '✅Para saber más del bot /acerca \n'
            start_msg+= '✅Para saber los comandos es /ayuda\n│\n'
            start_msg+= '🤩Que tu estancia disfutes del bot🤩\n'
            ...
            if loged:
                ...
                client.logout()
                bot.editMessageText(message,'Archivos Eliminados🗑️')
            else:
                bot.editMessageText(message,'🤔')
                message = bot.sendMessage(update.message.chat.id,'⚠️Error y posibles causas:\n1-Revise su Cuenta\n2-Servidor Desabilitado: '+client.path)
            ...
            jdb.save()
            statInfo = infos.createStat(username,getUser,jdb.is_admin(username))
            bot.editMessageText(message,"✅Configuración Aplicada")
